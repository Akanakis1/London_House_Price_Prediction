...
sample_submission = pd.read_csv('C:/Users/alexa/OneDrive/Έγγραφα/Work_Python/Data_Analysis/Project 3 London House Price Prediction/sample_submission.csv')
...
inspect(sample_submission, "Sample Submission dataset")
...
best_model_name = min(results, key=lambda name: results[name]['Root Mean Squared Error'])
...
def create_submission(best_model, test_df, features, id_col='ID', filename='submission.csv'):
    ...
    print(MAE(sample_submission['price'], np.log(submission_df['price'])))
